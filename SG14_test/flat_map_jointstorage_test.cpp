#include "SG14_test.h"
#include "flat_map_jointstorage.h"
#include <assert.h>
#include <deque>
#include <functional>
#include <list>
...
static void ExtractDoesntSwapTest()
{
    ...
    // This test fails if extract() is implemented in terms of swap().
    {
        ...
        stdext::flat_map<int, int, std::less<>, std::pmr::vector<std::pair<int, int>>> fs({{1, 10}, {2, 20}}, a);
        auto ctr = std::move(fs).extract();
        assert(ctr.get_allocator() == a);
    }
#endif
    ...
    {
        std::allocator<int> a;
        stdext::flat_map<int, int, std::less<>, std::vector<std::pair<int, int>>> fs({{1, 10}, {2, 20}}, a);
        auto ctr = std::move(fs).extract();
        assert(ctr.get_allocator() == a);
    }
}
...
static void DeductionGuideTests()
{
    ...
    if (true) {
        ...
        static_assert(std::is_same_v<decltype(fm2), flat_map<std::string, int, std::less<std::string>, std::deque<std::pair<std::string, int>>>>);
        // flat_map(std::list(...)) is ill-formed
#if __cpp_lib_memory_resource
        std::pmr::vector<std::pair<std::pmr::string, int>> pv;
        flat_map fm4(pv);
        static_assert(std::is_same_v<decltype(fm4), flat_map<std::pmr::string, int, std::less<std::pmr::string>, std::pmr::vector<std::pair<std::pmr::string, int>>>>);
#endif
    ...
    if (true) {
        // flat_map(KeyContainer, MappedContainer) does not exist
        std::vector<int> vi {2,1};
        std::vector<std::string> vs {"a","b"};
        assert(!flatmap_is_ctadable_from(0, vi, vs));
        assert(!flatmap_is_ctadable_from(0, std::move(vi), std::move(vs)));
    }
    if (true) {
        ...
        assert(!flatmap_is_ctadable_from(0, pv, std::allocator<int>()));
#endif
    }
    if (true) {
        ...
        static_assert(std::is_same_v<decltype(fm2), flat_map<std::string, int, std::less<std::string>, std::deque<std::pair<std::string, int>>>>);
        // flat_map(sorted_unique, std::list(...)) is ill-formed
#if __cpp_lib_memory_resource
        std::pmr::vector<std::pair<std::pmr::string, int>> pv;
    ...
    if (true) {
        ...
        assert(!flatmap_is_ctadable_from(0, stdext::sorted_unique, pv, std::allocator<int>()));
#endif
    }
    if (true) {
...
static void SpecialMemberTest()
{
    ...
    static_assert(std::is_nothrow_move_constructible<FS>::value == std::is_nothrow_move_constructible<typename FS::container_type>::value && std::is_nothrow_move_constructible<typename FS::key_compare>::value, "");
...
void sg14_test::flat_map_jointstorage_test()
{
    AmbiguousEraseTest();
    ...
    {
        using FS = stdext::flat_map<int, const char*, std::less<int>, std::deque<std::pair<int, const char*>>>;
        ConstructionTest<FS>();
        SpecialMemberTest<FS>();
        InsertOrAssignTest<FS>();
        ComparisonOperatorsTest<FS>();
    ...
    {
        using FS = stdext::flat_map<int, const char*, std::less<int>, std::pmr::vector<std::pair<int, const char*>>>;
        ConstructionTest<FS>();
        SpecialMemberTest<FS>();
        InsertOrAssignTest<FS>();
        ComparisonOperatorsTest<FS>();
    ...
    {
        using FS = stdext::flat_map<int, std::pmr::string, std::less<int>, std::pmr::vector<std::pair<int, std::pmr::string>>>;
        ConstructionTest<FS>();
        SpecialMemberTest<FS>();
        InsertOrAssignTest<FS>();
        ComparisonOperatorsTest<FS>();
